...
@Component
/**
 * automatically registered with OAuth2LoginReactiveAuthenticationManager
 * in reactive spring-security there is no ProviderManager or AuthenticationProvider counterpart. there is only ReactiveAuthenticationManager.
 * the input is result of tokenUri endpoint
 * the result will be set in SecurityContextHolder as a principal object
 */
public class ReactiveUserService implements ReactiveOAuth2UserService<OAuth2UserRequest, OAuth2User> {
    private final SecretKeyReader secretKeyReader;
    private final UserInfoAccessor userInfoAccessor;
    private final Logger logger= LoggerFactory.getLogger(this.getClass());

    public ReactiveUserService(SecretKeyReader secretKeyReader,
    ...
    public Mono<OAuth2User> loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        try{
            ...
            return Mono.just(userInfoAccessor.getIdentity(nationalId));
        } catch (Exception ex){
